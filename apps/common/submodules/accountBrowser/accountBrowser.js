	...
	var accountBrowser = {
		...
		accountBrowserBindEvents: function(args) {
			...
			template.find('.account-browser-search').on('keyup', function(e) {
				...
				// if only 1 in list and presses enter take this account
				if(template.find('.account-list-element:visible').length === 1 && e.which == 13) {
					var accountElement = template.find('.account-list-element:visible').first();
					if(accountElement) {
			...
			accountList.on('click', '.account-children-link:not(.disabled)', function() {
				...
				template.find('.account-browser-search').prop('disabled', false).val('');
				self.accountBrowserRenderList({
					...
					if(currentAccountId !== topAccountId) {
						...
						self.callApi({
							...
							success: function(data, status) {
								...
								template.find('.account-browser-search').focus();
								}
							}
						});
					}
		...
		accountBrowserRenderList: function(args) {
			...
			self.accountBrowserGetData(searchValue, parentId, function(data) {
                                // if 0 of customers in this account go back and show last tree
                                if(data.data.length == 0) {
                                        var currentAccountId = self.accountId,
                                        topAccountId = allowBackOnMasquerading ? monster.apps.auth.originalAccount.id : self.accountId;
