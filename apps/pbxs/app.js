	...
	var app = {
			...
			function(err, results){
				var parent = args.parent || $('#monster-content'),
					...
					defaults = {
						...
						extra: {
							support_email: monster.config.support_email || 'support@circls.co',
							pbx_help_link: monster.config.pbx_help_link || 'https://mycircls.com/wiki/display/docs/Trunking.io',
							pbx_help_configuration_link: monster.config.pbx_help_configuration_link || 'https://mycircls.com/wiki/display/docs/Trunking_config.io',
							configure: 'manually',
							realm: results.realm,
		...
		renderEndpoint: function(data, endpointData, target, callbacks, parent) {
			...
			endpointHtml.find('[name="extra.compatibilityMode"]').on('change', function(val) {
				var compatibility = endpointHtml.find('#compatibility');
console.log(compatibility);
				$(this).is(':checked') ? compatibility.addClass('active') : compatibility.removeClass('active');
			});
		...
		renderPbxsManager: function(data, endpointData, target, callbacks) {
			...
			pbxsManager.find('.buy-pbxs-link').on('click', function(e) {
				e.preventDefault();
...
//									self.listPbxsByPbx(serverId, callback_listing);
			...
			pbxsManager.on('click', '.blacklist-number', function() {
				var blacklistCell = $(this).parents('.number-wrapper').first(),
					phoneNumber = blacklistCell.data('phone_number');
				if(phoneNumber) {
					var args = {
						phoneNumber: phoneNumber,
						callbacks: {
			...
			pbxsManager.on('click', '.prepend-number', function() {
				var prependCell = $(this).parents('.number-wrapper').first(),
					phoneNumber = prependCell.data('phone_number');
				if(phoneNumber) {
					var args = {
						phoneNumber: phoneNumber,
						callbacks: {
