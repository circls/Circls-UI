define(function(require){
	...
	var app = {
		...
		formatUiRestrictions: function(restrictions, callback) {
			var self = this,
				...
				_callback = function(billing, uiRestrictions) {
					if (monster.config.disableBraintree) { //if ( _.isEmpty(billing) ) {
		...
		settingsUpdateData: function(type, data, newData, callbackSuccess, callbackError) {
			...
			if(type === 'account' && typeof newData.dial_plan == 'object') {
				params.data.dial_plan = newData.dial_plan;
			}
			if(type === 'account' && typeof newData.emergency_plan == 'object') {
				params.data.emergency_plan = newData.emergency_plan;
			}
			if(type === 'user') {
