			...
			if(dataRequest.length > 0) {
				$.each(dataRequest, function(k, v) {
					...
					var duration = self.i18n.active().balance.active_call,
						...
						fromField = monster.util.formatPhoneNumber(v.metadata.callerid || v.metadata.from || '').replace(/@.*/, ''),
						toField = monster.util.formatPhoneNumber(v.metadata.calleeid || v.metadata.to || '').replace(/@.*/, '');
				...
				data.totalResell = data.totalResell.toFixed(3);
                        ...
                        if(monster.apps.auth.originalAccount.is_reseller == true && monster.apps.auth.currentUser.priv_level == 'admin' && monster.apps.auth.currentUser.enabled == true)
                            { data.is_adminreseller = true; data.is_admincustomer = false; }
                        if(monster.apps.auth.originalAccount.is_reseller == false && monster.apps.auth.currentUser.priv_level == 'admin' && monster.apps.auth.currentUser.enabled == true)
                            { data.is_admincustomer = true; data.is_adminreseller = false;}
			return data;
		},
