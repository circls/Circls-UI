		...
		_render: function(container) {
			...
			self.getrates(function(data) {
				...
				if(monster.util.isMasquerading() == true || monster.util.isSuperDuper() == true)
					ratesArray.is_showaddButton = true;
		...
		renderAddPopUp: function() {
			...
			addratesTemplate.find("#save").on('click', function(e) {
				self.checkFormData(function(formData) {
					...
					if(monster.util.isMasquerading() != true && monster.util.isSuperDuper() != true) {
						toastr.error(monster.template(self, '!' + "("+data.rate_name+") " + self.i18n.active().rates.toastr.addresellernotMasquarde ));
						return;
					}
				...
				editratesTemplate.find(".save").on('click', function(e) {
					self.checkFormData(function(formData) {
						...
						self.updateArates(ratesId, formData, function(data) {
							...
							if(monster.util.isMasquerading() != true && monster.util.isSuperDuper() != true)
								toastr.warning(monster.template(self, '!' + "("+data.rate_name+") " + self.i18n.active().rates.toastr.editresellernotMasquarde ));
							else
