define(function(require){
	...
	var app = {
		...
		usersBindEvents: function(template, parent, data) {
				...
				monster.util.checkVersion(currentUser, function() {
					if(monster.ui.valid(form)) {
						...
						monster.parallel({
							...
							function(error, results) {
								toastr.success(monster.template(self, '!' + toastrMessages.userUpdated, { name: results.user.last_name + ' ' + results.user.first_name }));
				...
				pinTemplate.find('.save-new-pin').on('click', function() {
					...
					if(monster.ui.valid(form)) {
						self.usersUpdateVMBox(vmboxData, function(data) {
							...
							toastr.success(monster.template(self, '!' + toastrMessages.pinUpdated, { name: currentUser.last_name + ' ' + currentUser.first_name }));
						});
					}
				});
				...
				passwordTemplate.find('.save-new-username').on('click', function() {
					...
					if(monster.ui.valid(form)) {
						...
						self.usersUpdateUser(userToSave, function(userData) {
							...
							toastr.success(monster.template(self, '!' + toastrMessages.userUpdated, { name: userData.data.last_name + ' ' + userData.data.first_name }));
						});
					}
				});
			...
			template.on('click', '.regextern-number', function() {
				var regexternCell = $(this).parents('.item-row').first(),
					phoneNumber = regexternCell.data('id');
			...
			template.on('click', '.failover-number', function() {
				...
				if(phoneNumber) {
					var args = {
						...
						callbacks: {
							success: function(data) {
								if('failover' in data.data && data.data.failover.enabled) {
			...
			template.on('click', '.blacklist-number', function() {
				var blacklistCell = $(this).parents('.item-row').first(),
					phoneNumber = blacklistCell.data('id');
				...
				if(phoneNumber) {
					var args = {
						...
						callbacks: {
							success: function(data) {
								if('blacklist' in data.data && data.data.blacklist.enabled) {
									blacklistCell.find('.features i.feature-blacklist').addClass('active');
								} else {
									blacklistCell.find('.features i.feature-blacklist').removeClass('active');
								}
							}
						}
					};

					monster.pub('common.blacklist.renderPopup', args);
				}
			});
			...
			template.on('click', '.feature[data-feature="call_recording"]', function() {
				self.usersGetMainCallflow(currentUser.id, function(callflow) {
					if(callflow) {
						...
						toastr.error(self.i18n.active().users.callRecording.noNumber, '', {"timeOut": 10000});
					}
				});
			});
		...
		usersRenderFindMeFollowMe: function(params) {
			...
			if(params.userCallflow) var groupName = params.userCallflow.flow.data.name;
		...
		usersRenderCallRecording: function(params) {
			var self = this,
				...
				switchFeature = featureTemplate.find('.switch-state'),
				switchDBrecord = featureTemplate.find('.switch-dbrecord'),
			...
			switchFeature.on('change', function() {
				if($(this).prop('checked') && switchDBrecord.prop('checked')) {
					featureTemplate.find('.content').slideDown();
					featureTemplate.find('.content_url').slideUp();
				} else if($(this).prop('checked') && switchDBrecord.prop('checked') == false) {
					featureTemplate.find('.content').slideDown();
					featureTemplate.find('.content_url').slideDown();
				} else if($(this).prop('checked') == false && switchDBrecord.prop('checked') == false) {
					featureTemplate.find('.content').slideUp();
					featureTemplate.find('.content_url').slideUp();
				} else if($(this).prop('checked') == false && switchDBrecord.prop('checked')) {
					featureTemplate.find('.content').slideUp();
					featureTemplate.find('.content_url').slideUp();
				}
			});

			switchDBrecord.on('change', function() {
...
console.log(params.currentUser.smartpbx);
					if(!('call_dbrecording' in params.currentUser.smartpbx)) {
						params.currentUser.smartpbx.call_dbrecording = {
							enabled: false
						};
					}

					// enable the callflow for recording
		...
		usersCleanUserData: function(userData) {
			var userData = $.extend(true, {}, userData),
				...
				defaultCallerIdName = monster.util.convertUtf8ToAscii(fullName.substring(0, 15)),
				newCallerIDs = {
					caller_id: {
						internal: {
		...
		usersFormatCreationData: function(data, callback) {
			var self = this,
				...
				callerIdName = monster.util.convertUtf8ToAscii(fullName.substring(0, 15)),
				formattedData = {
					user: $.extend(true, {}, {
						caller_id: {
							internal: {
				...
				function(error, results) {
					...
					monster.parallel(listFnDelete, function(err, resultsDelete) {
						self.usersDeleteUser(userId, function(data) {
							toastr.success(monster.template(self, '!' + self.i18n.active().users.toastrMessages.userDelete, { name: data.last_name + ' ' + data.first_name }));
							self.usersRender();
						});
					});
				}
				...
				if(vmboxes.length > 0) {
					if(needVMUpdate) {
						self.usersGetVMBox(vmboxes[0].id, function(vmbox) {
							vmbox.name = user.last_name + ' ' + user.first_name + '\'s VMBox';
					...
					var vmbox = {
						...
						name: user.last_name + ' ' + user.first_name + '\'s VMBox'
					};
		...
		usersUpdateConferencing: function(data, globalCallback) {
			...
			monster.parallel({
					conference: function(callback) {
						var baseConference = {
							name: data.user.last_name + ' ' + data.user.first_name + ' SmartPBX Conference',
		...
		usersUpdateCallflowFaxing: function(data, newNumber, callflow, callback) {
			...
			if( callflow.hasOwnProperty('id') ) {
				...
				var defaultFaxbox = {
						name: user.last_name.concat(' ', user.first_name, self.i18n.active().users.faxing.defaultSettings.nameExtension),
