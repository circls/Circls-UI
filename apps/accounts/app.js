	...
	var app = {
		...
		renderAccountInfoStep: function(params) {
			...
			language.populateDropdown(parent.find('#accountsmanager_new_account_language'));
                        // regional dialplan
                        dialplan.regional.populateDropdown(parent.find('#accountsmanager_account_regiodialplan'));
                        // emergency dialplan
                        dialplan.emergency.populateDropdown(parent.find('#accountsmanager_account_emergencyplan'));
		...
		renderChargingStep: function(params) {
			var self = this,
				parent = params.parent,
				formattedClassifiers = $.map(params.classifiers, function(val, key) {
					return {
				...
				stepTemplate = self.getChargingTabContent({
					parent: parent,
					formattedClassifiers: formattedClassifiers
				});
		...
		*/
		editAccount: function(params) {
			...
			if($.isNumeric(templateData.account.created)) {
				templateData.account.created = monster.util.toFriendlyDate(accountData.created, "short");
                        ...
                        // regiodialplan
                        if(typeof accountData.regiodialplan == "string" && typeof monster.apps.auth.regiodiallocal[accountData.regiodialplan] == "string")
                                templateData.regiodialplanlocal = monster.apps.auth.regiodiallocal[accountData.regiodialplan];
                        // emergencydialplan
                        if(typeof accountData.emergencyplan == "string" && typeof monster.apps.auth.emergencydiallocal[accountData.emergencyplan] == "string")
                                templateData.emergencydialplanlocal = monster.apps.auth.emergencydiallocal[accountData.emergencyplan];
				...
				var $this = $(this),
					...
					newData = self.cleanFormData(monster.ui.getFormData('form_'+fieldName));
                                        ...
                                        if(typeof newData.regiodialplan == "string") {
                                                newData.dial_plan = {},
                                                newData.dial_plan = JSON.parse(dialplan.regional.list[newData.regiodialplan].dial_plan);
                                        }
                                        // store emergencydialplan
                                        if(typeof newData.emergencyplan == "string") {
                                                newData.emergency_plan = {},
                                                newData.emergency_plan = JSON.parse(dialplan.emergency.list[newData.emergencyplan].dial_plan);
                                        }
			...
			contentHtml.find('#accountsmanager_account_language').chosen({search_contains: true, width: "220px"});
                        ...
                        // regional dialplan
                        dialplan.regional.populateDropdown(contentHtml.find('#accountsmanager_account_regiodialplan'), accountData.regiodialplan);
                        contentHtml.find('#accountsmanager_account__regiodialplan').chosen({ search_contains: true, width: '220px' });

                        // emergency dialplan
                        dialplan.emergency.populateDropdown(contentHtml.find('#accountsmanager_account_emergencyplan'), accountData.emergencyplan);
                        contentHtml.find('#accountsmanager_account__emergencyplan').chosen({ search_contains: true, width: '220px' });
			...
			self.renderChargingTab({
				accountData: accountData,
				limits: accountLimits,
				balance: accountBalance,
				formattedClassifiers: formattedClassifiers,
				servicePlan: currentServicePlan,
				parent: contentHtml.find('#accountsmanager_charging_tab')
			});
		...
		/** Expected params:
			...
			- parent
		*/
		renderChargingTab: function(params) {
			var self = this,
				parent = params.parent,
				limits = params.limits,
				balance = params.balance,
				accountData = params.accountData,
				tabContentTemplate = self.getChargingTabContent(params);
			...
			parent.find('#accountsmanager_charge_save').click(function(e) {
				e.preventDefault();

				var twowayTrunksDiv = tabContentTemplate.find('.trunks-div.twoway'),
					newTwowayValue = twowayTrunksDiv.find('.slider-div').slider('value'),
					...
					chargeOnnet = tabContentTemplate.find('.charge-onnet-ckb').is(':checked');
					allowIncall = tabContentTemplate.find('.allow-incall-ckb').is(':checked');
					sayunderLimit = tabContentTemplate.find('.say-underlimit-ckb').is(':checked');
					maxpostpayAmount = tabContentTemplate.find('.max-postpay-amount-ckb').val();
					reserveAmount = tabContentTemplate.find('.reserve-amount-ckb').val();

				if(monster.ui.valid(parent.find('#accountsmanager_chargerestrictions_form'))) {

					callRestrictions = monster.ui.getFormData('accountsmanager_chargerestrictions_form').limits.call_restriction,
					$.each(params.formattedClassifiers, function(k, v) {
					    if(v.id && callRestrictions)
							...
							self.callApi({
								...
								data: {
									...
									data: $.extend(true, {}, limits, {
										...
										charge_onnet: chargeOnnet,
										allow_incall: allowIncall,
										say_underlimit: sayunderLimit,
										reserve_amount: reserveAmount,
										max_postpay_amount: maxpostpayAmount
									})
								},
								success: function(data, status) {
			...
			parent.find('#accountsmanager_chargerestrictions_form').append(tabContentTemplate);
		},
		...
		*/
		renderLimitsTab: function(params) {
			var self = this,
				...
				limitsDiv = tabContentTemplate.find('.limits-tab-container');
			...
			parent.find('#accountsmanager_limits_save').click(function(e) {
				...
				var newTwowayValue = twowayTrunksDiv.find('.slider-div').slider('value'),
					...
					newAllowPrepay = limitsDiv.find('.allow-prepay-ckb').val(),
					newAllowPostpay = limitsDiv.find('.allow-postpay-ckb').val();
		...
		getLimitsTabContent: function(params) {
			var self = this,
				...
				resource_consuming_calls = limits.resource_consuming_calls || 2,
		 ...
		 * This function is shared by both the edition tab and the creation wizard step.
		 */
		getChargingTabContent: function(params) {
			var self = this,
				formattedClassifiers = params.formattedClassifiers,
				limits = params.limits || {},
				template = $(monster.template(self, 'chargingTabContent', {
					mode: params.hasOwnProperty('accountData') ? 'update' : 'create',
					balance: params.balance || 0,
					...
					chargeOnnet: limits.hasOwnProperty('charge_onnet') ? limits.charge_onnet : false,
					sayunderLimit: limits.hasOwnProperty('say_underlimit') ? limits.say_underlimit : false,
					allowIncall: limits.hasOwnProperty('allow_incall') ? limits.allow_incall : false,
					reserveAmount: limits.hasOwnProperty('reserve_amount') ? limits.reserve_amount : '',
					maxpostpayAmount: limits.hasOwnProperty('max_postpay_amount') ? limits.max_postpay_amount : '',
					disableBraintree: monster.config.disableBraintree
				}));
			monster.ui.tooltips(template);
			return template;
