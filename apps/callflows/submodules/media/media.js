		...
		mediaRender: function(data, target, callbacks) {
			...
			$('.media-save', media_html).click(function(ev) {
				...
				if(monster.ui.valid(mediaForm)) {
					...
					self.mediaSave(form_data, data, function(_data, status) {
						if(!form_data.tts) {
							if($('#upload_div', media_html).is(':visible') && $('#file').val() != '') {
								if(file === 'updating') {
									toastr.error(self.i18n.active().callflows.media.the_file_you_want_to_apply, '', {"timeOut": 10000});
								}
								else {
					...
					toastr.error(self.i18n.active().callflows.media.there_were_errors_on_the_form, '', {"timeOut": 10000});
				}
			});
