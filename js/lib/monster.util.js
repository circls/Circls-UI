define(function(require){
	...
	var util = {
		...
		formatPhoneNumber: function(phoneNumber){
                        ...
                        firstL = phoneNumber.substr(0,1);
                        if(phoneNumber && monster.config.formatPhoneNumbers == "europe") {
                                if(phoneNumber.length >= 4 && phoneNumber.length <= 6) {
                                    ...
                                    }else if(firstL.match(/^[2-9]/g)) {
                                        phoneNumber = phoneNumber.replace(/(\d{2})(\d{2})(\d{3})(\d{3})/, '+$1 ($2) $3 $4');
                                    } else
                                        phoneNumber = phoneNumber.replace(/(\w{3})(\d{3})(\d{2})(\d{2})/, '$1 $2 $3 $4');
                                }
                                else if(phoneNumber.length === 11) {
                                    if(firstL.match(/^[2-9]/g)) {
                                        phoneNumber = phoneNumber.replace(/(\d{2})(\d{2})(\d{3})(\d{2})(\d{2})/, '+$1 ($2) $3 $4 $5');
                                    } else
                                    ...
                                    if(firstL.match(/^[2-9]/g)) {
                                        phoneNumber = phoneNumber.replace(/(\d{2})(\d{2})(\d{3})(\d{2})(\d{2})/, '+$1 ($2) $3 $4 $5');
                                    } else if(phoneNumber.substr(0,1) === "+" && phoneNumber.length === 12) {
                                        phoneNumber = phoneNumber.replace(/(\+)(\d{2})(\d{2})(\d{3})(\d{2})(\d{2})/, '$1 ($2) $3 $4 $5 $6');
                                    } else
                        ...
                        // else for USA
                            ...
                            else if(phoneNumber.substr(0,1) === "1" && phoneNumber.length === 11) {
			...
			return phoneNumber;
                ...
                capaitalizeSentence: function(Sentence){
		...
		*/
		...
		convertUtf8ToAscii: function (str) {
                    ...
                    conversions['Ae'] = 'Ä|À|Á|Â|Ã|Ä|Å|Ǻ|Ā|Ă|Ą|Ǎ';
                    ...
                    conversions['oe'] = 'ö|ò|ó|ô|õ|ō|ŏ|ǒ|ő|ơ|ø|ǿ|º';
                    conversions['R'] = 'Ŕ|Ŗ|Ř';
                    conversions['r'] = 'ŕ|ŗ|ř';
                    ...
                    conversions['T'] = 'Ţ|Ť|Ŧ';
                    conversions['t'] = 'ţ|ť|ŧ';
                    conversions['U'] = 'Ü|Ù|Ú|Û|Ũ|Ū|Ŭ|Ů|Ű|Ų|Ư|Ǔ|Ǖ|Ǘ|Ǚ|Ǜ';
                    conversions['u'] = 'ü|ù|ú|û|ũ|ū|ŭ|ů|ű|ų|ư|ǔ|ǖ|ǘ|ǚ|ǜ';
                    conversions['Y'] = 'Ý|Ÿ|Ŷ';
                    conversions['y'] = 'ý|ÿ|ŷ';
                    conversions['W'] = 'Ŵ';
